...
func checkContainerAgenticInnerHygiene(w io.Writer, hostName string, h *host.Host, runners []config.RunnerConfig, r *Result) {
	for _, pair := range containerAgenticInstallTargetsForHost(runners, hostName) {
		...
		if len(failures) == 0 {
			printLine(w, sevOK, hostName, fmt.Sprintf("container(agent): awf installed, inner Docker clean, and host.docker.internal reachable (bridge + AWF agent shim route) (%s)", cname))
			continue
		}
		for _, f := range failures {
			sev := sevWarn
