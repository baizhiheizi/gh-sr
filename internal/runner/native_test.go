...
func Test_windowsStartNative_usesCimProcessCreateForMergedLogs(t *testing.T) {
	...
	for _, addr := range []string{config.LocalAddr, "u@h"} {
		...
		if !strings.Contains(script, "Get-CimClass") || !strings.Contains(script, "Win32_ProcessStartup") || !strings.Contains(script, "ShowWindow=0") {
			t.Fatalf("addr=%s: should use Get-CimClass Win32_ProcessStartup with ShowWindow=0 to hide the console window: %q", addr, script)
		}
