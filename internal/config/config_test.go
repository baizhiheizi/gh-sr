...
func TestValidate_containerRunnerImage_toolcache(t *testing.T) {
	...
	cases := []struct {
		...
		{"valid explicit strip 1", []ContainerToolcacheEntry{{URL: "https://example.com/jdk.tar.gz", Dir: "jdk-temurin-17", Strip: 1}}, false},
		{"valid strip max", []ContainerToolcacheEntry{{URL: "https://example.com/t.tar.gz", Dir: "d", Strip: maxContainerRunnerToolcacheStripComponents}}, false},
		{"negative strip", []ContainerToolcacheEntry{{URL: "https://example.com/t.tar.gz", Dir: "d", Strip: -1}}, true},
		{"strip over max", []ContainerToolcacheEntry{{URL: "https://example.com/t.tar.gz", Dir: "d", Strip: maxContainerRunnerToolcacheStripComponents + 1}}, true},
