...
// TestColumnWidths covers the printer helper that sizes table columns to the
// longest cell in either the header or any row. Three cases matter:
//
//   - empty rows (no body): widths come purely from the headers
//   - short body cells: header widths dominate
//   - long body cells: the longest cell in each column wins
//   - ragged rows (fewer cells than headers): the row's bounds are respected,
//     no out-of-bounds panic
//   - nil cell in a body row: nil cells are ignored (no panic, no growth)
func TestColumnWidths(t *testing.T) {
	...
	t.Run("ragged row does not panic and stops at header length", func(t *testing.T) {
		...
		// First row has fewer cells than headers. columnWidths iterates
		// j over row and guards with `j < len(widths)`, so a ragged row
		// is safe and contributes nothing to later columns.
		rows := [][]string{{"short"}}
		got := columnWidths(headers, rows)
		want := []int{5, 1, 1}
		assertIntSlice(t, "widths", got, want)
	...
	t.Run("unicode cell width uses byte length", func(t *testing.T) {
		t.Parallel()
		// columnWidths uses len() (byte length), not rune count. We assert
		// the current contract: 3-byte UTF-8 runes count as 3. A future
		// change to rune-based width would be a behavioural change worth
		// catching in code review.
		headers := []string{"A"}
		rows := [][]string{{"☃"}}
		got := columnWidths(headers, rows)
		// "☃" is 3 bytes in UTF-8
...
// TestPrintRow verifies the per-row printer: it pads each cell to the
// column width (right-aligned, since %-*s pads on the right), emits
// a 2-space gutter after every cell including the last, and ends with
// a newline. Cells beyond len(widths) are dropped silently.
func TestPrintRow(t *testing.T) {
	...
	t.Run("truncates cells beyond the widths slice", func(t *testing.T) {
		...
		// First two cells printed; "c" and "d" silently dropped.
		// Format "%-*s  " with width=1: "a" (1 char, no pad) + "  " (gutter) = "a  ".
		// Same for "b". Concat: "a  b  " + newline.
		got := buf.String()
		want := "a  b  \n"
		if got != want {
			t.Errorf("got %q want %q", got, want)
		...
		if strings.Contains(got, "c") || strings.Contains(got, "d") {
			t.Errorf("trailing cells leaked: %q", got)
		}
	})
	...
	t.Run("empty cells are still padded and printed", func(t *testing.T) {
		...
		got := buf.String()
		// "" padded to width 2 = "  ", then "  " (gutter) = "    " (4 chars).
		// "x" padded to width 1 = "x", then "  " (gutter) = "x  " (3 chars).
		// Concat: "    x  " + newline.
		want := "    x  \n"
		if got != want {
			t.Errorf("got %q want %q", got, want)
...
// assertIntSlice is a tiny equality helper that produces a stable error
// message regardless of slice length.
