...
| `container_runner_image.toolcache` | Optional list of tool tarballs pre-extracted into the image tool cache at build time (`url:` + `dir:` per entry, optional `complete:`, at most 64). `url` must be https; `dir` is a relative path under `/home/runner/.toolcache` (no `..`, absolute paths, backslashes, or whitespace); the tarball is extracted as-is into `dir`. A zero-byte marker (default `dir.complete`, override with `complete:` — e.g. the ruby-builder tarballs ship a top-level `x64/` wrapper, so use `dir: Ruby/<ver>` + `complete: Ruby/<ver>/x64.complete`) marks the tool "already installed", which `ruby/setup-ruby` (and similar actions) honor by skipping their runtime download — useful on networks where GitHub release downloads fail intermittently. Runtime installs keep working for anything not listed. Changing the list changes the image tag/layout revision, so existing containers need `gh sr rebuild <name>`. |
