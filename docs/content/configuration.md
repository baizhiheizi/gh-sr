...
| `runners[].labels` | Labels for workflow `runs-on` matching. Include **`gh-aw`** when the runner should serve [GitHub Agentic Workflows](https://github.github.com/gh-aw/); that label also triggers **`gh sr doctor`** agentic prerequisite checks (with **`profile: agentic`**, or alone on native setups). |
| `runners[].mode` | `docker` or `native` (default: `docker` for Linux hosts, `native` for macOS and Windows). Set `docker` for Linux container runners on Windows (Docker Desktop) or macOS (Docker Desktop, OrbStack, or Colima). For **Linux** + gh-aw with **`mode: native`**, the account that runs the runner needs non-interactive **`sudo`**; see [Native Linux runners and sudo](host-setup.md#native-linux-runners-and-sudo-gh-aw). |
