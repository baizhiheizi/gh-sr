...
| `runners[].profile` | Optional. `agentic` auto-configures for [GitHub Agentic Workflows](https://github.github.com/gh-aw/): sets `mode: docker`, `docker_network_mode: host`, `docker_cap_add: [NET_ADMIN]`, and adds an `agentic` label. See [Host setup — GitHub Agentic Workflows](host-setup.md#github-agentic-workflows-gh-aw). |
