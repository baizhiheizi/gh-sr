...
| `runners[].runner_mode` | `native` (default) or `container`. `container` runs each runner instance in its own privileged Docker container with an inner dockerd (DinD), providing full isolation of `/tmp/gh-aw`, iptables rules, and MCP gateway port 80 between concurrent jobs on the same host. The image is built locally by `gh sr setup` and includes Docker CE, dnsmasq, and the actions runner. See [§8b in agentic-workflows.md](guides/agentic-workflows.md#8b-runner-modes-native-vs-container-recommended-for-concurrency). |
| `runners[].agentic_mcp_ports` | Optional list of TCP ports (length must equal `count`) for the MCP gateway on each instance. Only for `profile: agentic` + `runner_mode: native`. Mutually exclusive with `agentic_mcp_port_base`. |
| `runners[].agentic_mcp_port_base` | Optional base port for MCP gateway instances (base, base+1, …). Only for `profile: agentic` + `runner_mode: native`. Mutually exclusive with `agentic_mcp_ports`. |
