...
- **Docker CE** (the inner `dockerd` for DinD) with a **baked daemon config** that pins the inner default bridge gateway to `172.17.0.1` and points container DNS at a bundled **dnsmasq**, so `host.docker.internal` resolves deterministically for inner containers. The inner `dockerd` starts exactly once — there is no runtime DNS rewrite or daemon restart.
- The **actions runner**, **gh-aw CLI**, and **AWF** (`github/gh-aw-firewall`), plus `gh`, Node, Python (`uv`), Chromium/`chromium-chromedriver`, and common build/system libraries.
...
- for agentic instances: `awf` is available via `sudo` inside the container, `host.docker.internal` resolves to a non-loopback address from inner containers, the AWF service-routing bypass is installed, and there are no orphan `gh-aw`/`awf-`/`gh-aw-mcpg-*` containers or stale `DOCKER-USER` rules in the inner Docker.
