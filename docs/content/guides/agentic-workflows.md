...
- for agentic instances: `awf` is available via `sudo` inside the container, `host.docker.internal` resolves to a non-loopback address from inner containers, the runner's `/etc/resolv.conf` is pinned to the bundled dnsmasq (so the agent sandbox inherits an AWF-exempt resolver — flagged as `container-inner-resolv` otherwise), the AWF service-routing bypass is installed, and there are no orphan `gh-aw`/`awf-`/`gh-aw-mcpg-*` containers or stale `DOCKER-USER` rules in the inner Docker.
