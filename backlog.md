...
- 2026-08-07 container-status pattern: when the orchestrator delegates to a single h.Run shell script, have the mock simulate the script's stdout directly (e.g. `|configImage|digest|imageRev\n` for `containerLocalStatusOneShot`) rather than running individual `docker inspect` / `docker image inspect` sub-commands. The mock cannot execute shell, but the parse layer (`parseContainerStatusInspectOutput` + `formatContainerImageBuild`) is exercised end-to-end.
