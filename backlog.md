...
- `runPerHostParallel`, `ResolveHostInfo`, and `CollectHostMetrics` are all 100% covered.
...
### 3. `internal/runner/container.go` (37%) — large lifecycle functions, real Docker work
### 4. `internal/runner/native.go` — 21 untested fns, mostly Docker exec wrappers

## Low value (defer)

### 5. `internal/tui/*` (4.9%) — UI rendering
### 6. `cmd/gh-sr/*` (0%) — tested via `internal/ops`
### 7. `internal/autostart` (17.2%) — side-effect install helpers
...
- 2026-06-15: `CollectHostMetrics` 0% → 100%; `internal/ops` 33.4% → 37.6% (+4.2 pp).
- 2026-06-14: `ResolveHostInfo` 0% → 100%; `internal/ops` 24.2% → 33.4% (+9.2 pp).
