...
### 1. `internal/diskschedule` (14.2%) — needs `exec.Command` injection refactor
...
### 2. `internal/runner` (56.7%) — broader package wins
- Pure helpers covered 2026-07-02. Orchestrators at 0% (setupNative, startNative, etc.) need real Manager + httptest — high complexity, lower marginal ROI.

### 3. `internal/autostart` (94.7%) — incremental: `Uninstall` arm defaults, `instanceFromServiceBasename` 83.3%, `IsStale` 90.9%
- CleanupStale 76.5% and a few ~85-90% lines remain. Lower priority.
...
- **WriteRemoteBytes is base64-over-Run, not Upload** (2026-07-04): assert on the SSH command substring (`base64 -d`/powershell), not on `mock.LastUpload`. The install path uses the SSH channel for the file payload.

## Completed (do not re-do)

- 2026-07-04: Install 64.3→100, installSystemdUser 80→100, installSystemdSystem 77.8→100, installLaunchd 0→100, installWindowsTask 0→100; autostart 84.9→94.7 (+9.8 pp). Phantom.
